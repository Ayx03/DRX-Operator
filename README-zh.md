...
- **多行编辑器**：Enter 发送，Shift+Enter 或 Ctrl+J 换行；粘贴保留换行、缩进和空格。输入 `/` 显示命令候选，上下键选择，Tab 补全并循环候选；Enter 先补全未完成的命令，再按 Enter 执行。没有候选弹层时，上下键仅在视觉首末行切换历史，其他时候移动光标。
...
- **状态栏**：保留模式、费用和 Token 用量，空间允许时显示当前模型；窄窗口隐藏次要信息。

常用快捷键：`Ctrl+P` / `F1` 命令菜单，`Alt+M` 模型选择，`Ctrl+T` 完整记录与搜索，`Ctrl+B` Agent 工作台，`Ctrl+L` 回到最新，`F2` 运行指标，`Ctrl+S` 停止任务。Esc 优先收起命令候选（保留输入）、关闭弹窗或恢复被替换的草稿，否则停止当前任务。
...
`/model` 或顶部“模型”按钮打开可搜索的模型菜单，可按模型、提供方或主机搜索，并按提供方筛选；Enter 确认，Esc 取消且保留草稿。列表来自已配置的服务及其模型列表接口，不虚构全局模型目录；接口不支持或发现失败时仍保留配置模型和当前模型，并显示提示。`/model <模型名或提供方/模型名>` 唯一匹配时直接切换，同名模型或模糊匹配不唯一时进入菜单选择。

模型切换从下一次主 Agent 或 Worker 的模型请求生效，不中断正在返回的响应，也不改变该请求的重试和回退链。会话快照保存所选模型但不保存凭据，恢复时要求对应服务仍按原来源配置。菜单显示的是模型输出能力，实际请求仍按接口和输出预算策略裁剪。
...
| `/mode` | 查看当前模式 |
| `/model` | 打开可搜索的模型选择菜单 |
| `/model <模型名或提供方/模型名>` | 切换模型；匹配不唯一时打开菜单 |
| `/model current` | 输出当前模型、服务主机和能力 |
| `/model list [关键词]` | 发现并输出可用模型，可按关键词筛选 |
| `/model refresh` | 刷新可用模型并打开菜单 |
